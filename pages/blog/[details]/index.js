    ...
    <div className={styles.textContainer}>
      {paragraphs?.slice(paragraphs.length / 2).map((paragraph, index) => (
        ...
        <article className={styles.tagsandsm}>
         ...
         <div style={{display:'flex',gap:'3px'}}> <button onClick={shareOnWhatsApp}><Image src={WP}/></button>
            <button onClick={shareOnInstagram}><Image src={Insta}/></button>
            <button onClick={shareOnLinkedIn}><Image src={Linkedin}/></button>
          <button onClick={shareOnX}><Image src={X}/></button></div>
